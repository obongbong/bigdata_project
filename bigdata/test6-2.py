...
# 5. 모델 정의 및 앙상블
rf   = RandomForestRegressor(n_estimators=RF_EST, max_depth=RF_DEPTH, random_state=42, n_jobs=-1)
xgb  = XGBRegressor(n_estimators=XGB_EST, max_depth=XGB_DEPTH, objective='reg:squarederror', random_state=42)
...
plt.scatter(y_true, y_pred, color='tomato', alpha=0.5)
plt.plot([min(y_true), max(y_true)], [min(y_true), max(y_true)], 'k--')  # y = x 기준선
plt.xlabel("실제 NOx")
plt.ylabel("예측 NOx")
